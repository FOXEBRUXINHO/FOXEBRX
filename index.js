	...
	client.on('group-participants-update', async (anu) => {
		...
		try {
			...
			if (anu.action == 'add') {
				...
				teks = `Seja bem-vindo@${num.split('@')[0]}\não seja um ghost 😳 *${mdata.subject}*`
		...
		try {
			...
			mess = {
				...
				only: {
					...
					admin: '❌ Este comando só pode ser usado pelos adm!!membro comum ❌',
			...
			switch(command) {
				...
				case 'bc':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Não e mais um membro comum 😳@${mentioned[0].split('@')[0]} Mais respeito ok?`, mentioned, true)
				...
				case 'demote':
					...
					if (mentioned.length > 1) {
						...
						mentions(`Não e mais um adm 👅@${mentioned[0].split('agora e um membro comum!`, mentioned, true)
				...
				case 'add':
					...
					try {
						...
						console.log('Error :', e)
						reply('Falha ao adicionar alvo, talvez porque é privado ')
					}
					break
				case 'kick':
					if (!isGroup) return reply(mess.only.group)
				...
				case 'listadmins':
					if (!isGroup) return reply(mess.only.group)
					teks = `Lista dos adm do grupo*${groupMetadata.subject}*\nTotal : ${groupAdmins.length}\n\n`
					no = 0
